			...
			u64 len, int mf_flags)
{
	...
	/*
	 * Record the error on the specific daxdev only (per-daxdev). The
	 * resolver rejects extents that reference an errored daxdev via
	 * famfs_dax_err(), so only files that use this device are affected.
	 */
 ...
 * dax_dev_get() reference is released and the slot is left invalid. @name may
 ...
 */
static void famfs_devlist_free(struct famfs_fs_info *fsi, struct super_block *sb)
{
				 ...
				 struct dax_device *dax_devp)
{
	...
	pr_err("%s: memory error on unrecognized daxdev\n", __func__);
}
...
famfs_get_tree(struct fs_context *fc)
{
	...
	/* slot-0 alias; the resolver still uses fsi->dax_devp for now */
	fsi->dax_devp = fsi->dax_devlist->devlist[0].devp;
	...
	if (!sb->s_root) {
		...
		err = -ENOMEM;
		/* famfs_kill_sb() -> famfs_devlist_free() releases the daxdev */
		goto deactivate_out;
	}
