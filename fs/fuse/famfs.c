 ...
 *
 * An interleaved extent spreads its logical bytes round-robin across nstrips
 * strips at chunk_size granularity: chunk c lives in strip (c % nstrips) at
 * stripe (c / nstrips). Walk the interleaved extents to find the one that
 * contains @file_offset, then compute the strip and the offset within it.
 */
static int
famfs_interleave_fileofs_to_daxofs(struct inode *inode, struct iomap *iomap,
				   loff_t file_offset, off_t len,
				   unsigned int flags)
{
	struct fuse_inode *fi = get_fuse_inode(inode);
	...
	/* Outer loop is over the interleaved extents */
	for (i = 0; i < meta->fm_niext; i++) {
		struct famfs_meta_interleaved_ext *fie = &meta->ie[i];
		u64 chunk_size = fie->fie_chunk_size;
		u64 nstrips    = fie->fie_nstrips;
		u64 ext_size   = min_t(u64, fie->fie_nbytes, meta->file_size);
		...
		if (local_offset < ext_size) {
			...
			/* never map past the end of the strip's backing extent */
		...
		local_offset -= ext_size; /* beyond this striped extent */
	}
 ...
 *
 * PMD (2 MiB) faults additionally depend on the mapping being 2 MiB-aligned.
 * famfs-fuse does not install its own file_operations; it relies on the
 * generic fuse_file_operations.get_unmapped_area == thp_get_unmapped_area
 * (fs/fuse/file.c) to align mappings. If that hook were ever dropped, a
 * 2 MiB-aligned fmap would silently fault at PTE (4 KiB) granularity instead
 * of PMD.
 *
