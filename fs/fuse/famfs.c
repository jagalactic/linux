				...
				const char *name)
{
	...
	scoped_guard(rwsem_write, &fc->famfs_devlist_sem) {
		...
		if (rc) {
			 ...
			 * with the daxdev already held by *this* fuse_conn means a
			 * concurrent acquire won and will publish the slot valid:
			 * not an error, and must not be cached as dax_err. Any
			 * other failure (foreign holder, not a dax device, wrong
			 * driver type) is permanent for this connection, so record
			 * dax_err to stop re-fetching and re-acquiring it.
			 */
...
int famfs_daxdev_open(struct fuse_conn *fc, struct fuse_backing_map *map)
{
	...
	return famfs_install_daxdev(fc, map->daxdev_index, devno, NULL);
}
	...
	const struct famfs_file_meta *meta)
{
	...
	/* Collect indices of missing daxdevs while holding reader lock */
