...
static int fsdev_dax_zero_page_range(struct dax_device *dax_dev, pgoff_t pgoff,
				    size_t nr_pages)
{
	...
	while (resid > 0) {
		...
		long len = __fsdev_dax_direct_access(dax_dev, poff,
						   nr_pages, DAX_ACCESS, &kaddr,
						   NULL);
		len = min_t(long, len, PAGE_SIZE);
		fsdev_write_dax(kaddr, ZERO_PAGE(0), offset, len);
