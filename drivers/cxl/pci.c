...
static int cxl_pci_map_regblock(struct pci_dev *pdev,
...
static void cxl_pci_unmap_regblock(struct pci_dev *pdev, struct cxl_register_map *map)
{
			  ...
			  struct cxl_register_map *map)
{
	...
	rc = cxl_pci_map_regblock(pdev, map);
	if (rc)
		return rc;

	rc = cxl_probe_regs(pdev, map);
	cxl_pci_unmap_regblock(pdev, map);
...
static int cxl_pci_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	if (rc)
		return rc;
#if 1
	rc = cxl_mu_mem_enumerate_cmds(cxlm);
#else
